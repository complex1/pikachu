  ...
  <div class="api-card mt-4 bg-ele-1 round">
    <div :class="`api-card-${api.method} p-4`">
      <div class="api-card-header v-center space-between">
        <div class="v-center">
          <h4 class="method-badge py-2 mr-4 tx round" @click="open = !open">{{api.method}}</h4>
          <h4 class="api-path pointer tx-lg tx-light" @click="open = !open">{{ api.path }}</h4>
        </div>
        ...
        <div class="api-card-impl__request flex">
          <div class="tx-light">
            ...
            <div class="tx-small mt-3 flex flex-end">
              ...
              <p-btn v-if="edit" color="green"  light class="ml-2" @click="applyApiCall"> Apply </p-btn>
              <p-btn v-if="edit" color="lightBlue" light class="ml-2" @click="setProperty"> Reset </p-btn>
              <p-btn v-if="edit" color="red" @click="edit = false" light class="ml-2"> Cancel </p-btn>
            </div>
          </div>
          <div class="tx-light">
    ...
    applyApiCall () {
      const requestConfig = {
        ...
        data: {
          ...
          query: this.query.reduce((q, e) => {q[e.name] = e.value; return q}, {}),
          params: this.pathParam.reduce((q, e) => {q[e.name] = e.value; return q}, {}),
          headers: this.headers.reduce((q, e) => {q[e.name] = e.value; return q}, {}),
          body: this.body
        }
      }
